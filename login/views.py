...
def custom_page_not_found(request, exception):
    messages.error(
        request,
        "The page you were looking for was not found.You will redirected to this page.",
    )
...
def send_assistance(request):
    try:
        ...
        if not result:
            return JsonResponse(
                {"success": False, "error": "Failed to send message to admin"}
            )
...
def generate_otp(request, user_id=None):
    ...
    if request.method == "POST":
        ...
        if is_valid:
            ...
            if device_hash:
                TrustedDevice.objects.get_or_create(user=user, hash=device_hash)
            ...
            if is_first_login:
                ...
                PasswordReset.objects.create(
                    user=user, token=reset_token, created_at=timezone.now(), used=False
                )
            ...
            print(
                f"OTP verification failed. Retry: {current_otp.retry}, Total failed: {current_otp.total_failed_attempts}"
            )
    ...
    return render(request, "login/otp.html", {"user_id": user_id})
...
def user_dashboard(request):
    ...
    upcoming_webinar = WebinarAttendees.objects.filter(
        user=request.user, webinar__start_date__gte=today
    )
    past_webinar = WebinarAttendees.objects.filter(
        user=request.user, webinar__start_date__lt=today
    )
    upcoming_messages = ""
    history_messages = ""
    ...
    return render(
        request,
        "login/user_nav/user_dashboard.html",
        {
...
def calendar(request):
    ...
    return render(
        request, "login/user_nav/calendar.html", {"upcoming_webinars": upcoming_webinar}
    )
...
def event_data(request):
    ...
    for webinar in webinars:
        data.append({"title": webinar.title, "start": webinar.start_date.isoformat()})
...
def certificate(request):
    ...
    ongoing_webinars = Webinar.objects.filter(
        until_date__gt=today, attendees__user=request.user
    ).prefetch_related("attendees")

    completed_webinars = Webinar.objects.filter(
        until_date__lt=today, attendees__user=request.user
    ).prefetch_related("attendees")
    ...
    return render(
        request,
        "login/user_nav/certificate.html",
        {
...
def admin_calendar(request):
    ...
    return render(
        request,
        "login/admin_panel/calendar.html",
...
def admin_certificate(request):
    ...
    return render(
        request,
        "login/admin_panel/certificate.html",
        {"webinars": webinars, "today": today},
    )
...
def admin_users(request):
    ...
    return render(request, "login/admin_panel/users.html", {"all_users": users})
...
def create_admin(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            ...
            messages.success(
                request,
                f"Staff account for '{first_name} {last_name}' created successfully.",
            )
    ...
    if request.method == "POST":
        ...
        if username and username.strip():
            if User.objects.filter(username=username).exclude(id=user.id).exists():
                ...
                return render(
                    request, f"login/{redirected}", {"user_taken": user_taken}
                )
...
def change_password(request):
    if request.method == "POST":
        ...
        if not errors:
            ...
            redirected = (
                "admin_panel/setting.html"
                if (request.user.is_superuser or request.user.is_staff)
                else "user_nav/user_setting.html"
            )
            return render(
                request,
                f"login/{redirected}",
...
def log_list(request):
    ...
    logs = (
        LogEntry.objects.filter(user__is_staff=True)
        ...
        if log.action_flag == DELETION:
            if log.content_type:
                ...
                if (
                    ...
                    log_data["enhanced_message"] = (
                        f"Deleted {model_name} (ID: {log.object_id})"
                    )
                else:
                    log_data["enhanced_message"] = (
                        f"Deleted {model_name}: {log.object_repr}"
                    )
            else:
            ...
            log_data["enhanced_message"] = (
                log.change_message or log.object_repr or "No details available"
            )
        ...
        if request.method == "POST":
            ...
            if not new_password or len(new_password) < 6:
                ...
                return render(
                    ...
                    {"token": token, "is_first_login": is_first_login},
                )
...
def cal_event_data(request, id):
    ...
    results["overall"] = (
        round(sum(all_values) / len(all_values), 2) if all_values else 0
    )
...
def get_completed_events(request):
    ...
    try:
        ...
        completed_webinars = (
            Webinar.objects.filter(until_date__lt=now, evaluation__isnull=False)
            .annotate(response_count=Count("evaluation", distinct=True))
        ...
        events_data = [
            {
                ...
                "title": webinar.title,
                "date": (
                    webinar.start_date.strftime("%B %d, %Y")
...
def compare_events(request):
    ...
    if not event1_id or not event2_id:
        return JsonResponse(
            {"error": "Both event1 and event2 parameters are required"}, status=400
        )

    if event1_id == event2_id:
        return JsonResponse(
            {"error": "Cannot compare an event with itself"}, status=400
        )
    ...
    try:
        ...
        comparison_data = {
            "event1": {
                ...
                "title": event1.title,
                "date": (
                    event1.start_date.strftime("%B %d, %Y")
                    if event1.start_date
                    else "No date"
                ),
            ...
            "event2": {
                ...
                "title": event2.title,
                "date": (
                    event2.start_date.strftime("%B %d, %Y")
...
def get_event_details(request, event_id):
    ...
    try:
        ...
        event_data = {
            ...
            "time": (
                webinar.time.strftime("%I:%M %p")
                if hasattr(webinar, "time") and webinar.time
                else "No time"
            ),
...
def get_event_statistics(request, event_id):
    ...
    try:
        ...
        if not evaluations.exists():
            return JsonResponse(
                {"event_title": webinar.title, "total_responses": 0, "statistics": None}
            )
        ...
        for question_key in question_stats:
            ...
            if stats["responses"] > 0:
                total_score = sum(
                    rating * count for rating, count in stats["distribution"].items()
                )
...
def update_user_form(request):
    ...
    try:
        ...
        if (
            request.user.id == user_to_edit.id
            and request.user.is_superuser
            and request.POST.get("role") != "admin"
        ):
