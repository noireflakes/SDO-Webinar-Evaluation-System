...
class UserProfile(models.Model):
    ...
    img = models.ImageField(
        ...
        null=True,
        blank=True,
        default="UserProfile/cloud_emwloy.png",
    )
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            try:
                ...
                if (
                    old_instance.img
                    and old_instance.img != self.img
                    and old_instance.img.name != "UserProfile/defaultprofile_ntxlw1.jpg"
                ):
...
class TrustedDevice(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="trusted_device"
    )
