...
def test_result(request, web_id, type, id):
    ...
    test_responses = TestResponse.objects.filter(
        user=id, question__webinar=webinar, question__test_type=type
    )
    score = 0
    for response in test_responses:
        ...
        if question.question_type == "MC":
            ...
            if response.user_input == correct:
                score += 1
    TestResult.objects.create(webinar=webinar, user=user, test=type, score=score)
...
def display_result(request, id):
    ...
    return render(
        request,
        "exam_portal/statistics.html",
        {"webinar": webinar, "comments": comments},
    )
...
def attendees_data(request, id):
    ...
    for attendee in attendees:
        questionnaire_response = ResponseQuestionaire.objects.filter(
            webinar=webinar, user=attendee.user
        ).first()
        ...
        if attendee.user:
            ...
            if attendee.user.first_name or attendee.user.last_name:
                user_name = (
                    f"{attendee.user.first_name} {attendee.user.last_name}".strip()
                )
            else:
        ...
        attendees_list.append(
            {
                ...
                "evaluation_timestamp": (
                    questionnaire_response.timestamp if questionnaire_response else None
                ),
            }
        )
    ...
    attendees_list.sort(
        key=lambda x: (
            ...
            -(
                x["evaluation_timestamp"].timestamp()
                if x["evaluation_timestamp"]
                else 0
            ),
        )
    )
...
def rounded_data(request, id):
    ...
    evaluations = {
        "speaker": [
            speaker_counter.get(5, 0),
            speaker_counter.get(4, 0),
            speaker_counter.get(3, 0),
            speaker_counter.get(2, 0),
            speaker_counter.get(1, 0),
        ],
        "venue": [
            venue_counter.get(5, 0),
            venue_counter.get(4, 0),
            venue_counter.get(3, 0),
            venue_counter.get(2, 0),
            venue_counter.get(1, 0),
        ],
        "meals": [
            meal_counter.get(5, 0),
            meal_counter.get(4, 0),
            meal_counter.get(3, 0),
            meal_counter.get(2, 0),
            meal_counter.get(1, 0),
        ],
        "manage": [
            manage_counter.get(5, 0),
            manage_counter.get(4, 0),
            manage_counter.get(3, 0),
            manage_counter.get(2, 0),
            manage_counter.get(1, 0),
        ],
        "overall": [
            overall_counter.get(5, 0),
            overall_counter.get(4, 0),
            overall_counter.get(3, 0),
            overall_counter.get(2, 0),
            overall_counter.get(1, 0),
        ],
    }
    print(evaluations)
...
def result_data(request, id):
    ...
    for user_email, data in user_evaluations.items():
        ...
        speaker_avg = (
            sum(data["speaker"]) / len(data["speaker"]) if data["speaker"] else 0
        )
        ...
        category_scores = [
            s for s in [speaker_avg, venue_avg, meal_avg, manage_avg] if s > 0
        ]
        if category_scores:
    ...
    # attendance rate
    attendance_rate = (
        (total_attended / total_registered * 100) if total_registered > 0 else 0
    )

    # evaluation completion rate
    evaluation_rate = (
        (total_evaluations / total_attended * 100) if total_attended > 0 else 0
    )
    ...
    category_averages = {
        "speaker": (
            sum(speaker) / len(speaker)
            if speaker and any(s > 0 for s in speaker)
            else 0
        ),
        ...
        "manage": (
            sum(manage) / len(manage) if manage and any(s > 0 for s in manage) else 0
        ),
    }
    ...
    response_data = {
        ...
        "summary": {
            ...
            "test_data": {
                ...
                "pre_test_average": (
                    round(pre_test_data[1], 1)
                    if pre_test_data and pre_test_data[1]
                    else 0
                ),
                "post_test_count": post_test_data[0] if post_test_data else 0,
                "post_test_average": (
                    round(post_test_data[1], 1)
                    if post_test_data and post_test_data[1]
                    else 0
                ),
            },
        },
...
def test_data(request, id):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Failed to fetch test data: {str(e)}", "test_result": []},
            status=500,
        )
...
def generate_qr(request, id, type):
    ...
    if not qr:
        ...
        qr_db = TestQR.objects.create(test=webinar, type=type, name=f"QR_test_{id}")
        qr_db.img.save(f"QR_TEST_ID_{id}.png", ContentFile(qr_img))
...
def qr_evalution(request, id):
    ...
    if not qr:
        ...
        qr_db = EvalQR.objects.create(test=webinar, type=type, name=f"QR_test_{id}")
        qr_db.img.save(f"QR_TEST_ID_{id}.png", ContentFile(qr_img))
...
def display_qr(request, id, type):
    ...
    return render(request, "exam_portal/display_qr.html", {"qr": qr})
...
def create_certificate(request, id):
    ...
    try:
        ...
        return render(
            request, "exam_portal/createCertificate.html", {"webinar": webinar}
        )
...
def redirect_certificate(request, id):
    ...
    return render(
        request,
        "exam_portal/createCertificate.html",
        {"img": img, "webinar": img.webinar, "messages": messages},
    )
...
def cert_preview(request, id):
    ...
    return render(
        request, "exam_portal/certificatepreview.html", {"certificate": cert_json}
    )
